...
for gam_file_no = 11:11

%[100 101 102 103 110 112 113 114 115 116 117 121 129 137 147]
%for ratio_of_beta_c_to_epislon_b = [0.0001 0.0005 0.001 0.005 0.0025 0.007 0.01 0.025 0.05  0.1 1] 
...
save(append('approx',fileno,"j",index ,"modFBEMBc",strrep(string(round(beta_c,2,"significant")), '.', 'p'),"NoOmegaAndExtraTwo4sOldGAMh_s",strrep(string(round(h_s,2,"significant")), '.', 'p')));
...
[PosT,surface_type] = gam_topo_shell(gam_file_start,dx,L_w,L_h,D_off,f_p,L_ang);
...
for akai = 1:length(kappa_es)
    ...
    for akaj = 1:length(sigma_si)
        ...
        for akak = 1:length(sigma_s)
            ...
            [P_t_full,P_t_ml,P_t_full_comp,P_t_ml_comp] = Facet_Echo_Model_looped(op_mode,lambda,bandwidth,P_T,h,v,pitch,roll,prf,beam_weighting,G_0,D_0,gamma1,gamma2,N_b,t,PosT,...
	    				 						      surface_type,sigma_0_snow_surf,sigma_0_snow_vol,kappa_e,...
